...
export default function MerchantCashOut() {
  ...
  const [formData, setFormData] = useState({ agent: '', amount: '' });
  const [fieldErrors, setFieldErrors] = useState<FieldErrors>({ agent: '', amount: '' });
  ...
  const validate = (): boolean => {
    const errors: FieldErrors = { agent: '', amount: '' };
    let valid = true;

    if (!formData.agent.trim()) {
      errors.agent = 'Agent phone number is required.';
      valid = false;
    }
    ...
    <DashboardLayout>
      <Box>
        <Box sx={{ display: 'flex', alignItems: 'center', gap: 2, mb: 4 }}>
          ...
          <Box>
            <Typography sx={{ fontWeight: 800, fontSize: 22, color: '#1e293b', letterSpacing: '-0.3px' }}>Cash Out</Typography>
            <Typography sx={{ color: '#64748b', fontSize: 14 }}>Withdraw merchant earnings via an agent</Typography>
          </Box>
        </Box>
            ...
            <Box component="form" onSubmit={handleSubmit} noValidate>
              <Stack spacing={2.5}>
                ...
                <Button type="submit" fullWidth variant="contained" disabled={loading}
                  sx={{ height: 50, fontSize: 15, fontWeight: 700, textTransform: 'none', borderRadius: '12px', background: `linear-gradient(135deg, ${COLOR}, #7c3aed)`, boxShadow: `0 0 24px ${COLOR}40`, '&:hover': { background: `linear-gradient(135deg, #a78bfa, ${COLOR})` }, '&:disabled': { background: 'rgba(139,92,246,0.3)', color: 'rgba(255,255,255,0.4)' } }}>
                  {loading ? <><CircularProgress size={16} sx={{ color: '#fff', mr: 1 }} />Processing…</> : 'Cash Out →'}
                </Button>
              </Stack>
            </Box>
          </Box>
              ...
              <Box sx={{ display: 'flex', gap: 3, flexWrap: 'wrap' }}>
                {trnxId && (
                  <Box>
                    ...
                    <Chip label={trnxId} size="small" sx={{ mt: 0.5, bgcolor: 'rgba(139,92,246,0.15)', color: '#a78bfa', fontWeight: 700, fontSize: 12 }} />
                  </Box>
                )}
                {currentBalance !== null && (
                  <Box>
