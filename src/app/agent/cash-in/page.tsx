...
export default function CashIn() {
  ...
  const [formData, setFormData] = useState({ customer: '', amount: '' });
  const [fieldErrors, setFieldErrors] = useState<FieldErrors>({ customer: '', amount: '' });
  ...
  const validate = (): boolean => {
    const errors: FieldErrors = { customer: '', amount: '' };
    let valid = true;

    if (!formData.customer.trim()) {
      errors.customer = 'Customer phone number is required.';
      valid = false;
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const res = await API.post('/transaction/deposit', {
        from_account: phoneNumber, to_account: formData.customer, amount: Number(formData.amount),
      });
    ...
    <DashboardLayout>
      <Box>
        <Box sx={{ display: 'flex', alignItems: 'center', gap: 2, mb: 4 }}>
          ...
          <Box>
            <Typography sx={{ fontWeight: 800, fontSize: 22, color: '#1e293b', letterSpacing: '-0.3px' }}>Cash In</Typography>
            <Typography sx={{ color: '#64748b', fontSize: 14 }}>Deposit funds into a customer's wallet</Typography>
          </Box>
        </Box>
            ...
            <Box component="form" onSubmit={handleSubmit} noValidate>
              <Stack spacing={2.5}>
                ...
                <Button type="submit" fullWidth variant="contained" disabled={loading}
                  sx={{ height: 50, fontSize: 15, fontWeight: 700, textTransform: 'none', borderRadius: '12px', background: `linear-gradient(135deg, ${COLOR}, #0891b2)`, boxShadow: `0 0 24px ${COLOR}40`, '&:hover': { background: `linear-gradient(135deg, #22d3ee, ${COLOR})` }, '&:disabled': { background: 'rgba(6,182,212,0.3)', color: 'rgba(255,255,255,0.4)' } }}>
                  {loading ? <><CircularProgress size={16} sx={{ color: '#fff', mr: 1 }} />Processing…</> : 'Cash In →'}
                </Button>
              </Stack>
            </Box>
          </Box>
              ...
              <Box sx={{ display: 'flex', gap: 3, flexWrap: 'wrap' }}>
                {trnxId && (
                  <Box>
                    ...
                    <Chip label={trnxId} size="small" sx={{ mt: 0.5, bgcolor: 'rgba(6,182,212,0.15)', color: '#22d3ee', fontWeight: 700, fontSize: 12 }} />
                  </Box>
                )}
                {depositedAmount !== null && (
                  <Box>
                    ...
                    <Typography sx={{ fontWeight: 800, color: '#1e293b', fontSize: 18, mt: 0.5 }}>৳ {depositedAmount.toFixed(2)}</Typography>
                  </Box>
                )}
                {depositedAmount !== null && (
                  <Box>
                    <Typography sx={{ fontSize: 12, color: '#64748b', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.5px' }}>Service Fee</Typography>
                    <Typography sx={{ fontWeight: 800, color: '#1e293b', fontSize: 18, mt: 0.5 }}>৳ 0.00</Typography>
                  </Box>
                )}
                {currentBalance !== null && (
                  <Box>
